...
import { getWorkouts, saveWorkouts, getWorkoutById } from '@/lib/storage'
  ...
  if (mode === 'list') {
    return (
      <div className="flex flex-col min-h-dvh bg-black">
        ...
        <div className="fixed bottom-8 left-0 right-0 max-w-md mx-auto px-4">
          <button
            onClick={createNew}
            ...
            {editing.exercises.map((ex, i) => (
              <div key={ex.id} className="bg-[#111] border border-[#1a1a1a] rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#555] font-mono">{ex.muscle} · {ex.sets.length} series × {ex.sets[0]?.reps} reps</p>
                </div>
                <button
